class Switch {
  ...
  move() {
    if (this.on) this.x_ = this.x + this.r;
    else this.x_ = this.x - this.r;
  }
  colour() {
...
const marg = 50, //this decides the placing of 0.000 and toggles
  spacing = 25; //this is the spacing between each letter on screen
...
function mouseClicked(screenNumber) {
  screenNumber++;
}

function welcomeScreen(screenNumber) {
  if (screenNumber == 0) {
    mouseClicked(screenNumber);
  }
}
...
function draw() {
  ...
  if (reach === test.length) current = 0;
...
function keyTyped() {
  ...
  if (lost && restartSwitch.on) lost = false;
