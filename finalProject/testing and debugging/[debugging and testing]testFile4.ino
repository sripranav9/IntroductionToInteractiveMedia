...
int turnTime = 200;             //time the robot takes to turn after reversing back after facing an obstacle
  ...
  if (digitalRead(switchPin) == LOW) { //if the on switch is flipped
    ...
    if (distance < 15) {              //if an object is detected
      ...
      Serial.print("TURNING LEFT");
       ...
       rightMotor(255);
       leftMotor(0); //changed from -255 to 0
       delay(turnTime);
    }
      ...
      rightMotor(255); //move the right wheel forward in full speed
      leftMotor(150); //move the left wheel forward in full speed
    }
